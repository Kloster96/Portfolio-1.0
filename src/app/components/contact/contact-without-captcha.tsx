...
const ContactWithoutCaptcha = () => {
  ...
  return (
    <div className="relative group p-8 lg:p-10 rounded-3xl border border-white/5 bg-white/[0.02] backdrop-blur-3xl shadow-2xl">
      <div className="flex flex-col gap-8">
        <div className="flex flex-col gap-2">
          <h3 className="text-2xl font-bold text-white tracking-tight">
            {t.contact.send.replace(" Enviar Mensaje", "").trim()
              ? t.contact.send.replace(" Mensaje", "")
              : "Send Message"}
          </h3>
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-2 group/input">
            <label className="text-sm font-bold text-slate-500 uppercase tracking-widest ml-1 flex items-center gap-2 group-focus-within/input:text-cyan-400 transition-colors">
          ...
          <div className="flex flex-col gap-2 group/input">
            ...
            />
            {error.email && (
              <p className="text-xs text-cyan-400 ml-1">
          ...
          <div className="flex flex-col gap-4 mt-2">
            {error.required && (
              <p className="text-sm text-cyan-400 text-center font-medium">
                {t.contact.emptyFields}
              </p>
            )}

            <button
              className="relative group/btn overflow-hidden rounded-2xl bg-gradient-to-r from-cyan-500 to-blue-600 p-[1px] transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50"
              onClick={handleSendMail}
              disabled={isLoading}
            >
              <div className="relative flex items-center justify-center gap-2 bg-[#030712] group-hover/btn:bg-transparent transition-all rounded-[15px] px-8 py-4 text-white font-bold uppercase tracking-widest text-sm">
                {isLoading ? (
      ...
      {/* Decorative Accent */}
      <div className="absolute w-1 h-20 bg-gradient-to-b from-cyan-500 to-transparent left-0 top-20 rounded-full" />
    </div>
  );
};
